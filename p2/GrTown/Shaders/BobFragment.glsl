...
void main (void) {
  ...
  vec3 ambientColor = vec3(0.5,0.4,0.7);
  //color of material
  vec3 diffuseColor = vec3(0.62,0.47,0.933);
  //lower than 1,1,1 due to long distance from sun
  vec3 specularColor = vec3(0.7,0.5,1);
  ...
  if(sun == vec3(0)){
    ...
    tempLight = tempLight * lightIntensity;
  }
  color = vec4(tempLight, 0.8);
};